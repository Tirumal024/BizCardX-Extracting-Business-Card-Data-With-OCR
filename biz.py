...
# st.markdown("## :violet[ BizCardX: Extracting Business Card Data with OCR]")

# SETTING-UP BACKGROUND IMAGE
# def setting_bg():
#     st.markdown(f""" <style>.stApp {{
#                         background:url("https://images.pexels.com/photos/6985045/pexels-photo-6985045.jpeg?auto=compress&cs=tinysrgb&w=600");
...
with st.sidebar:
    ...
    st.image("https://www.klippa.com/wp-content/uploads/2023/08/processing-menu-page-s-1.gif")
...
if selected == "Home":
    ...
    with col1:
        ...
        st.image(r"https://media3.giphy.com/media/20iQNcNaKoKQAiPVlX/giphy.gif?cid=ecf05e472ssgag0t0xy4emsb6gq789py5bsxtk5b2m9f7uqq&ep=v1_gifs_search&rid=giphy.gif&ct=g", width=300)
    with col2:
        st.write(
            "## :violet[About :] Bizcard is a Python application designed to extract information from business cards.")
        st.write(
            '## The main purpose of Bizcard is to automate the process of extracting key details from business card images, such as the name, designation, company, contact information, and other relevant data. By leveraging the power of OCR (Optical Character Recognition) provided by EasyOCR, Bizcard is able to extract text from the images.')

# UPLOAD AND EXTRACT MENU
if selected == "Upload & Extract":
    if st.button(":violet[Already stored data]"):
        mycursor.execute(
        ...
        st.write(updated_df)
    st.subheader(":violet[Upload a Business Card]")
    ...
    if uploaded_card is not None:
        ...
        if st.button(":violet[View updated data]"):
            mycursor.execute(
                "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from card_data")
            updated_df = pd.DataFrame(mycursor.fetchall(),
                                      columns=["Company_Name", "Card_Holder", "Designation", "Mobile_Number",
...
# MODIFY MENU
if selected == "Modify":
    st.subheader(':violet[You can view , alter or delete the extracted data in this app]')
    ...
    if select == "ALTER":
        st.markdown(":violet[Alter the data here]")
    ...
    if select == "DELETE":
        st.subheader(":violet[Delete the data]")
        try:
            mycursor.execute("SELECT card_holder FROM card_data")
            result = mycursor.fetchall()
            business_cards = {}
            ...
            if selected_card == "None":
                ...
                st.write(f"### You have selected :violet[**{selected_card}'s**] card to delete")
